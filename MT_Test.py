...
def Python_MT (HashList_copy):
    ...
    for j, row in data.iterrows():
        mTree.add_leaf(str(HashList[j]), True)
    mTree.make_tree()
...
def main():
    ...
    m = My_Algo(h)
    end_time = time.time()
    print(color.BOLD + '\033[96m ' + "Total execution time: --- %s seconds ---" % (end_time - start_time) + color.END)
    validation(m, h)
    print(color.BOLD + '\033[93m' + "-----------------------------------------" + color.END)
    print(color.BOLD +'\033[93m' + "*** ----------------------------------------- ***" + color.END)
    ...
    Python_MT(h)
    end_time = time.time()
    print(color.BOLD + '\033[96m ' + "Total execution time of Python's MT: --- %s seconds ---" % (end_time - start_time) + color.END)
